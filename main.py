...
from modules import styles, data, signals, alerts
...
with st.sidebar:
    ...
    with st.expander("🔔 Alerts (Telegram)"):
        tg_token = st.text_input("Bot Token", type="password")
        tg_chat_id = st.text_input("Chat ID")
        if st.button("Test Alert"):
            alerts.send_telegram_alert(tg_token, tg_chat_id, "🔔 *Test Alert* from ProTrade Analytics!")

# --- MAIN APP ---
if ticker:
    stock, info, hist = data.get_stock_data(ticker, days_back)
    ...
    if info and not hist.empty:
        ...
        # --- ALERTS & SIGNALS ---
        active_signals = signals.calculate_signals(hist)
        if active_signals:
            st.subheader("🔔 Active Signals")
            sig_cols = st.columns(len(active_signals))
            for i, (icon, title, desc) in enumerate(active_signals):
                with sig_cols[i]:
                    st.info(f"**{icon} {title}**\n\n{desc}")
                    if st.button(f"Send Alert {i}", key=f"alert_{i}"):
                        alerts.send_telegram_alert(tg_token, tg_chat_id, f"🚨 *Signal Detected for {ticker}*\n\n{icon} {title}\n{desc}")
        ...
        with tab_news:
            ...
            news_items = data.get_news(stock)
            if news_items:
